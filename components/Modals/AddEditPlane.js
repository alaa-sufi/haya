...
export default function AddEditPlane({
  openModal,
  onCloseModal,
  add,
  edit,
  ...
  const onSubmit = (values) => {
    ...
    axios
      .post(add ? `${process.env.NEXT_PUBLIC_SERVER}/trip/air` : `${process.env.NEXT_PUBLIC_SERVER}/trip/${editId}`, values)
      .then((response) => {
        onCloseModal();
  ...
  return (
    <Modal
      ...
      centered
    >
      <HeaderField title={add ? "+ إضافة  رحلة طيران جديدة" : " تعديل رحلة الطيران "} />
      <Formik
        initialValues={edit ? { begin_date: convertDate(dataEdit.begin_date), from_city_id: dataEdit.from_city_id ,  to_city_id:dataEdit.to_city_id , category_id:"2", details: dataEdit.details}  :  { begin_date: "", from_city_id: "" ,   to_city_id:"",category_id:"2" , details:""}}
        onSubmit={onSubmit}
